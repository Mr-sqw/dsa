...
public class WorldCup {
	...
	public static void main(String[] args) {
		...
		double[] winRateOf8Arr = new double[16];
		//1/8决赛
		for (int i = 0; i < 16; i+=2) {
			winRateOf8Arr[i] =  arr[i][i+1] * (arr[i][i+2]+arr[i][i+3]);
		}
		for (int i = 1; i < 16; i+=2) {
			winRateOf8Arr[i] =  arr[i][i-1];
		}
		//1/4决赛
		double[] winRateOf4Arr = new double[16];
		for (int i = 0; i < 16; i+=2) {
			//winRateOf4Arr[i] =  winRateOf8Arr[i]*arr[];
		}
