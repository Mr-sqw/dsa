 ...
 */
public class ColaHappiness {

	public static void main(String[] args) {
		Scanner scanner = new Scanner(System.in);
		short n = scanner.nextShort();//要买的可乐数
		short m = scanner.nextShort();//小美喝的可乐数
		short k = scanner.nextShort();//可乐种数

		short[] happinessA = new short[k];
		short[] happinessB = new short[k];
		for (int i = 0; i < k; i++) {
			happinessA[i] = scanner.nextShort();
			happinessB[i] = scanner.nextShort();
		}
		scanner.close();
