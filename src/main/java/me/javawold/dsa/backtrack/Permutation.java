...
public class Permutation {
    ...
    private void doPermutation(char[] arr, LinkedHashSet<Integer> selectedSet, List<String> returnList) {
        ...
        for (int i = 0; i < arr.length; i++) {
            if (selectedSet.add(i)) {//1.从当前层“剩下”的可选择列表中 选择 一个元素来组成排列。 递归调用之前做选择。

                //2.继续从 以当前层以i为根的下一层“剩下”的可选择列表中选择一个元素来组成排列。 递归调用
                doPermutation(arr, selectedSet, returnList);

                //3.当前层以i为根的所有分支遍历完毕，继续以当前层其他节点为根寻找分支
                // 取消选择该元素。回溯，继续遍历，从“剩下”的可选择列表中选择一个元素来组成排列。 递归调用之后撤销选择。
                //递归调用返回，“方法栈”深度减少一层，取消上一次的选择，
